...
void Class::setPre(std::string pre) { this->preR = pre; }
...
void Class::checkStatus() {
  ...
  checkAttribute("available choices",
                 availableChoices == Choices::NoneSpecified ? "" : "a choice");
...
std::string Class::parseBasedOnCheck() {
  std::string message;
  if (this->hasPrereq) {
    message = "You cannot take this class yet. Take " + std::string(" take ") +
              this->preR + " instead";
  } else {
